...
- **Reels — single API (FetchSocial)**: `server.js` reels section rewritten — primary resolver `fetchsocial-all-in-one-scraper.p.rapidapi.com` (same `IG_KEY`; YT Shorts + TikTok + IG Reels via post URLs in `REEL_SOURCES` map; missing sports fall back to `all`), 60s throttle after 429, fallback to old instagram120 accounts flow, 15-min DB cache. NOTE: RapidAPI account was quota-exhausted (429 across all packages) during testing — feed returns empty until quota resets/upgrades; code verified subscribed (429 ≠ 403).
